...
"""
Utilities for exporting a GitHelp corpus to MMORE-compatible JSONL.

GitHelp uses DocumentRecord internally. MMORE expects input samples with a
"text" field, optional modalities, and metadata. This module bridges the two
formats so that a GitHelp corpus can be indexed with MMORE.
"""
...
def document_to_mmore_sample(doc: DocumentRecord) -> dict[str, Any]:
    ...
    The text field contains a small GitHelp metadata header followed by the
...
def export_corpus_to_mmore_jsonl(
    *,
    ...
    """
    Export a GitHelp JSONL corpus to MMORE's JSONL input format.

    Parameters
    ----------
    corpus_path:
        Path to the GitHelp corpus JSONL file.
    output_path:
